...
PackageInfo.prototype.get = function () {
    ...
    // Read package.json
    var packageJson = this._readPackageData(app.options.root + '/package.json');
    ...
    var apidocJson = this._readPackageData(app.options.root + '/apidoc.json');
...
PackageInfo.prototype._getHeaderFooter = function (json) {
    ...
    ['header', 'footer'].forEach(function (key) {
        if (json[key] && json[key].filename) {
            ...
            var filename = path.join(app.options.root, json[key].filename);
